...
pub struct AudioTrack {
    ...
    pub file_source: Option<track::source::FileSource>,
    pub sample_rate: u32,
    pub gain: f32,
    pub pan: f32,
    pub solo: bool,
...
impl AudioTrack {
    pub fn new(
        ...
        file_source: Option<track::source::FileSource>,
    ) -> Self {
        ...
        if let Some(stream) = &mut self.stream_source {
            if self.file_source.is_none() {
                if let Some(path) = path {
                    self.file_source =
                        Some(track::source::FileSource::new(&path, stream.sample_rate))
                } else {
                    panic!("Expected path");
                }
            }
    ...
    fn from(value: track::raw::AudioTrackRaw) -> Self {
        ...
        AudioTrack {
            ...
            file_source: if let Some(file_source_path) = value.file_source_path {
                Some(track::source::FileSource::new(
